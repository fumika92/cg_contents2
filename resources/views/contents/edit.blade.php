      ...
      <div class="bg-white py-6 sm:py-8 lg:py-12">
        <div class="max-w-screen-2xl px-4 md:px-8 mx-auto">
          ...
          <div class="mb-10 md:mb-16">
            <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-6">CG作品の編集</h2>
          </div>
          <!-- text - end -->
      
          <!-- form - start -->
          <form action="/contents/{{ $post->id }}" method="POST" enctype="multipart/form-data" class="max-w-screen-md grid sm:grid-cols-2 gap-4 mx-auto">
      	  @csrf
          @method('put')
      	  <!-- post - title -->
          <div class="sm:col-span-2">
            <label for="title" class="inline-block text-gray-800 text-sm sm:text-base mb-2">タイトル*</label>
            <input name="post[title]" value="{{ $post->title }}" class="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
          ...
          <div class="sm:col-span-2">
            ...
            <textarea name="post[body]" class="w-full h-64 bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2">{{ $post->body }}</textarea>
    		    <p class='body__error' style='color:red'>{{ $errors->first('post.body') }}</p>
          </div>
      	  
      	  <!-- post - image_path -->
      	  <div class="grid grid-cols-1 space-y-2">
            <label class="text-sm font-bold text-gray-500 tracking-wide">画像を添付</label>
            <div class="flex items-center justify-center w-full">
                <label class="flex flex-col rounded-lg border-4 border-dashed w-full h-60 p-10 group text-center">
                    ...
                    <input  name="image" value="{{ $post->image_path }}" type="file" class="hidden">
      	  ...
      	  <div class="relative inline-flex">
      		...
      		<select name="post[category_id]" size="1" class="h-10 pl-5 pr-10 focus:outline-none appearance-none">
